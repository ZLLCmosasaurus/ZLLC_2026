...
void Gimbal_Device_CAN1_Callback(Struct_CAN_Rx_Buffer *CAN_RxMessage)
{
    ...
    case (0x201):
    {
        chariot.Gimbal.Motor_Yaw_DJI3508.CAN_RxCpltCallback(CAN_RxMessage->Data);
    }
    break;
    case (0x202):
    {
        chariot.Gimbal.Motor_Pitch_DJI2006.CAN_RxCpltCallback(CAN_RxMessage->Data);
    }
    break;
    }
}
...
void DR16_UART3_Callback(uint8_t *Buffer, uint16_t Length)
{
    ...
    memcpy(Bluetooth_RxData, Buffer, Length);
...
void VT13_UART_Callback(uint8_t *Buffer, uint16_t Length)
{
    ...
    if (Bluetooth_RxData[0] == 0xAA && Bluetooth_RxData[1] == 0x55 && Bluetooth_RxData[2] == 0x0B)
    {
        Bluetooth_Data_Process(Bluetooth_RxData);
    }
}
...
void Task1ms_TIM5_Callback()
{
    ...
    if (start_flag == 1)
    {   
        if(chariot.MiniPC.Get_MiniPC_Status() == MiniPC_Status_DISABLE)
            buzzer_setTask(&buzzer, BUZZER_DEVICE_OFFLINE_PRIORITY);
        else
            buzzer_setTask(&buzzer, BUZZER_FORCE_STOP_PRIORITY);
        
        chariot.TIM_Control_Gimbal_PeriodElapsedCallback();
#ifdef GIMBAL
...
// UART_Init(&huart1, SuperCAP_UART1_Callback, 128);
    ...
    // 遥控器接收
    UART_Init(&huart3, DR16_UART3_Callback, Bluetooth_length);

#ifdef IMAGE_VT12
    UART_Init(&huart6, Image_UART6_Callback, 40);
#endif // VT12
#ifdef IMAGE_VT13
    UART_Init(&huart1, VT13_UART_Callback, Bluetooth_length);
#endif // VT12
