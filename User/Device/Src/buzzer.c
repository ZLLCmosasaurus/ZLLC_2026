...
BUZZER_RETURN_T buzzer_playPokemonHealed(buzzer_t *buzzer);
...
BUZZER_RETURN_T buzzer_taskScheduler(buzzer_t *buzzer)
{
	...
	case BUZZER_MARIO_SIMPLE_PRIORITY:
		...
		break;
	case BUZZER_POKEMON_HEALED:
		ret = buzzer_playPokemonHealed(buzzer);
...
{
	uint32_t current_tick, wait;
    buzzer_getTick(buzzer, &current_tick);
    wait = current_tick - buzzer->status.tickStart;

    // 前奏与三个 B5 的处理
    if (!buzzerBsp_checkTickTolerance(PKM_RUBY_PRE_A5, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_playTone(buzzer, 12); // 低音前奏 A5
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_STEP1_B5, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_playTone(buzzer, 14); // B5
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_GAP1, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_setState(buzzer, BUZZER_SWITCH_OFF); // 物理间隔
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_STEP2_B5, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_playTone(buzzer, 14); // B5
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_GAP2, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_setState(buzzer, BUZZER_SWITCH_OFF); // 物理间隔
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_STEP3_B5, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_playTone(buzzer, 14); // B5
        
    // 结尾旋律
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_STEP4_A5, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_playTone(buzzer, 12); // A5
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_STEP5_E6, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_playTone(buzzer, 19); // E6
    
    // 释放逻辑
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_OFF, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
        buzzer_setState(buzzer, BUZZER_SWITCH_OFF);
    else if (!buzzerBsp_checkTickTolerance(PKM_RUBY_RELEASE, wait, BUZZER_TASK_TICK_DIFFERENCE_TOLERANCE))
...
static uint8_t platform_setFreq(void *peripheral_handle, uint32_t channel, uint16_t value)
{
	...
	if (value < 0 || value > 65535)
		return BUZZER_WRONG_PARAM; // 适用于 16 bit ARR 寄存器的保护机制
